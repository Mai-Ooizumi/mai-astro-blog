...
<div class="lg:hidden">
  <button
    ...
    class="flex w-11 justify-center rounded-lg py-2 text-(--text-color) transition-all hover:bg-(--primary-color-hover) hover:text-(--primary-color)"
  >
...
<!-- mobile search bar -->
<div class="fixed w-full z-20 top-18 left-1/2 -translate-x-1/2">
  <!-- search input pannel -->
  <div
    bind:this={searchBar}
    class="absolute left-1/2 -translate-x-1/2 w-[95%] px-1 flex flex-col h-0 opacity-0 lg:hidden bg-(--card-color) rounded-xl transition-all overflow-hidden before:content-[''] after:content-[''] before:pt-1 after:pb-1"
  >
    <div class="bg-black/5 dark:bg-white/5 h-10 rounded-lg flex flex-row">
      ...
      <input
        ...
        class="text-(--text-color) grow bg-transparent outline-none transition-all"
  ...
  <div
    ...
    class="max-h-[436px] overflow-y-scroll opacity-0 absolute! left-1/2 -translate-x-1/2 h-0 w-[95%] bg-(--card-color) rounded-2xl top-14 transition-all"
  >
    <div
      ...
      {#each searchResult as item}
        ...
        >
          <div class="flex flex-row space-x-1 items-center">
            <p
              class="line-clamp-1 text-lg font-semibold text-(--text-color) result-title"
            >
              {item.meta.title}
            </p>
            <span class="text-(--primary-color) font-extrabold">
          ...
          <div>
            <div class="h-10">
              <p
                class="item-excerpt text-sm line-clamp-2 text-(--text-color-lighten)"
              >
                {@html item.excerpt}
              </p>
            </div>
